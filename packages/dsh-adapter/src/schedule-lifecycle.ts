...
export interface PlanScheduleChangeOptions {
  sources: DshProfileSources
  targetUri: string
}

export interface DshScheduleAddPlan extends DshScalarEditPlan {
  scheduleChange: typeof SCHEDULE_PLUGIN_ADD
  scheduleMode: 'add-plugin'
}

export interface DshScheduleRemovePlan extends DshScalarEditPlan {
  scheduleChange: typeof SCHEDULE_PLUGIN_REMOVE
  scheduleMode: 'remove-plugin'
}

export type DshScheduleChangePlan = DshScheduleAddPlan | DshScheduleRemovePlan
...
function selectedProfilePatch(
  ...
  if (layer?.owner !== 'profile') throw new Error('Schedule changes currently target only the profile patch')
...
function collectIds(node: unknown, ids: string[]): void {
  if (isAlias(node)) throw new Error('alias-backed inserted rows make Schedule changes ambiguous')
...
function loaderEntryIds(document: ReturnType<typeof parseDocument>): string[] {
  ...
  for (const patch of document.contents.items) {
    if (isAlias(patch)) throw new Error('top-level aliases make Schedule changes ambiguous')
...
function generatedText(newline: string): string {
  return [
    '- insert:',
    `    - id: ${SCHEDULE_PLUGIN_ADD.entryId}`,
    `      name: '${SCHEDULE_PLUGIN_ADD.pluginName}'`,
  ].join(newline)
}
...
function exactGeneratedInsert(document: ReturnType<typeof parseDocument>): { patch: unknown; entry: unknown } {
  if (!isSeq(document.contents) || document.contents.flow === true) {
    throw new Error('Schedule removal requires a block-style top-level YAML sequence')
  }
  const matches = document.contents.items.flatMap(patch => {
    const insert = mapField(patch, 'insert')
    if (!isSeq(insert)) return []
    return insert.items.some(entry => scalarString(mapField(entry, 'id')) === SCHEDULE_PLUGIN_ADD.entryId)
      ? [{ patch, insert }]
      : []
  })
  if (matches.length !== 1) throw new Error('Schedule removal requires one exact generated insert block')
  const match = matches[0]
  if (match === undefined) throw new Error('unreachable: generated Schedule insert is missing')
  exactMapKeys(match.patch, ['insert'], 'Schedule insert patch')
  if (match.insert.flow === true
    || match.insert.anchor !== undefined
    || match.insert.tag !== undefined
    || match.insert.items.length !== 1) {
    throw new Error('Schedule insert must remain a one-entry block sequence')
  }
  const entry = match.insert.items[0]
  exactMapKeys(entry, ['id', 'name'], 'Schedule entry')
  const id = mapField(entry, 'id')
  const name = mapField(entry, 'name')
  if (!isScalar(id) || id.value !== SCHEDULE_PLUGIN_ADD.entryId || id.tag !== undefined || id.anchor !== undefined
    || !isScalar(name) || name.value !== SCHEDULE_PLUGIN_ADD.pluginName || name.tag !== undefined || name.anchor !== undefined) {
    throw new Error('Schedule insert no longer matches the GraphControl-generated plugin entry')
  }
  return { patch: match.patch, entry }
}

/** Append the one current-official Schedule plugin entry without writing. */
export function planScheduleAdd(options: PlanScheduleChangeOptions): DshScheduleAddPlan {
  ...
  if (loaderEntryIds(document).includes(SCHEDULE_PLUGIN_ADD.entryId)) {
    throw new Error('Schedule is already declared in the selected profile patch')
  }
  ...
  if (loaderEntryIds(candidate).filter(id => id === SCHEDULE_PLUGIN_ADD.entryId).length !== 1) {
    throw new Error('Schedule addition did not produce one unique plugin entry')
  }
  const intent: SetDeclaredScalarIntent = {
    kind: 'set-declared-scalar',
    entryId: SCHEDULE_PLUGIN_ADD.entryId,
    path: ['name'],
    value: SCHEDULE_PLUGIN_ADD.pluginName,
  }
  ...
  const base = createEditPlan({
    id: `add-schedule:${encodeURIComponent(options.sources.profile)}`,
    targetUri: patch.uri,
    expectedBytes: patch.rawBytes,
    candidateBytes: new TextEncoder().encode(candidateText),
    summary: 'Enable Session-local reminders for new Agents',
    textChange: { startOffset, endOffset, beforeText, afterText },
    semanticChange: {
      kind: 'add-schedule',
      entryId: intent.entryId,
      pluginName: intent.value,
      securityDelta: SCHEDULE_PLUGIN_ADD.securityDelta,
    },
  })
  return {
    ...base,
    ...
    preview,
    previews: [preview],
    scheduleChange: SCHEDULE_PLUGIN_ADD,
    scheduleMode: 'add-plugin',
  }
}

/** Remove only the final, untouched Schedule insert created above. */
export function planScheduleRemove(options: PlanScheduleChangeOptions): DshScheduleRemovePlan {
  const patch = selectedProfilePatch(options.sources, options.targetUri)
  const document = parseForEdit(patch.text)
  const generated = exactGeneratedInsert(document)
  if (!isSeq(document.contents)) throw new Error('unreachable: Schedule patch is not a sequence')
  ...
  if (generatedIndex !== document.contents.items.length - 1) {
    throw new Error('Schedule removal requires the generated insert to remain the final patch block')
  }
  const startOffset = lineStartAt(patch.text, rangeStart(generated.patch, 'Schedule insert patch'))
  const endOffset = rangeEnd(generated.patch, 'Schedule insert patch')
  if (endOffset !== patch.text.length) {
    throw new Error('Schedule removal requires the generated insert to occupy the final source block')
  }
  const newline = insertionNewline(patch.text)
  ...
  if (beforeText !== expected) {
    throw new Error('Schedule removal accepts only the untouched GraphControl-generated insert')
  }
  ...
  if (!isSeq(candidate.contents)
    || loaderEntryIds(candidate).includes(SCHEDULE_PLUGIN_ADD.entryId)) {
    throw new Error('Schedule removal candidate still declares the plugin')
  }
  const intent: SetDeclaredScalarIntent = {
    kind: 'set-declared-scalar',
    entryId: SCHEDULE_PLUGIN_REMOVE.entryId,
    path: [],
    value: null,
  }
  const preview: YamlScalarEditPreview = {
    ...
    beforeText,
    afterText,
    previousValue: SCHEDULE_PLUGIN_REMOVE.pluginName,
    nextValue: null,
    candidateText,
    changed: true,
  }
  const base = createEditPlan({
    id: `remove-schedule:${encodeURIComponent(options.sources.profile)}`,
    targetUri: patch.uri,
    expectedBytes: patch.rawBytes,
    candidateBytes: new TextEncoder().encode(candidateText),
    summary: 'Remove Session-local reminders from new Agents',
    textChange: { startOffset, endOffset, beforeText, afterText },
    semanticChange: {
      kind: 'remove-schedule',
      entryId: intent.entryId,
      pluginName: SCHEDULE_PLUGIN_REMOVE.pluginName,
    },
  })
  return {
    ...base,
    ...
    preview,
    previews: [preview],
    scheduleChange: SCHEDULE_PLUGIN_REMOVE,
    scheduleMode: 'remove-plugin',
  }
}
